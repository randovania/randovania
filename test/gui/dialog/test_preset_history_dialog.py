...
async def test_select_item(skip_qtbot, default_preset, tmp_path, mocker):
    ...
    mock_prompt = mocker.patch("randovania.gui.lib.file_prompts.prompt_preset",
                               return_value=tmp_path.joinpath("sample.rdvpreset"))
...
def test_calculate_previous_versions(mocker):
    ...
    mock_old = mocker.patch("randovania.gui.dialog.preset_history_dialog._get_old_preset", side_effect=[
        "Invalid preset",
        preset_a := MagicMock(spec=Preset),
        preset_b := MagicMock(spec=Preset),
    ])
    mock_describe = mocker.patch("randovania.gui.dialog.preset_history_dialog._describe_preset", return_value=[
        "Part1", "Part2",
    ])
