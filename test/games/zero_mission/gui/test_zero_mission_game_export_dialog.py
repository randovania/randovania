...
from randovania.games.zero_mission.gui.dialog.game_export_dialog import MZMGameExportDialog
...
def test_on_output_file_button_exists(skip_qtbot, tmp_path, mocker, has_output_dir):
    ...
    if has_output_dir:
        ...
        expected_default_name = str(tmp_path.joinpath("output_path", "Zero Mission - MyHash"))
        output_path.mkdir()
    else:
        output_path = None
        expected_default_name = "Zero Mission - MyHash"

    options = MagicMock()
    options.per_game_options.return_value = MZMPerGameOptions(
        cosmetic_patches=MZMCosmeticPatches.default(),
        output_path=output_path,
    )

    window = MZMGameExportDialog(options, {}, "MyHash", True, [])
...
def test_on_output_file_button_cancel(skip_qtbot, tmp_path, mocker):
    ...
    # Assert
    mock_prompt.assert_called_once_with(window, "Zero Mission - MyHash.gba", [window.valid_file_type])
    assert window.output_file_edit.text() == ""
...
def test_save_options(skip_qtbot, tmp_path):
    ...
    # Assert
    assert options.per_game_options(MZMPerGameOptions).output_path == Path("somewhere")
...
def test_get_game_export_params(skip_qtbot, tmp_path, save_spoiler: bool):
    ...
    # Assert
    assert result == MZMGameExportParams(
        spoiler_output=tmp_path.joinpath("output", "Zero Mission - MyHash.rdvgame"),
        input_path=tmp_path.joinpath("input/game.gba"),
        output_path=tmp_path.joinpath("output", "Zero Mission - MyHash.gba"),
    )
