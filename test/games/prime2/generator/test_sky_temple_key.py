...
def _create_pool_with(shuffled_count: int, db: ResourceDatabase):
    return PoolResults(
        to_place=[
            sky_temple_keys.create_sky_temple_key(i, db)
            for i in range(shuffled_count)
        ],
        assignment={},
        starting=[
            sky_temple_keys.create_sky_temple_key(i, db)
            for i in range(shuffled_count, 9)
        ],
    )
...
def test_create_key_id(index: int, echoes_resource_database):
    ...
    assert key.progression == (
        (echoes_resource_database.get_item(f"TempleKey{index + 1}"), 1),
    )
