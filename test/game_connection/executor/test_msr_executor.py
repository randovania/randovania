...
from randovania.game_connection.executor.msr_executor import MSRExecutor, MSRLuaException, PacketType
...
async def test_connect(executor, mocker):
    ...
    api_request_answer = [
        b"\x03",
        b"\x01",
        b"\x01\x2b\x00\x00\x00",
        b"1,4096,00000000-0000-1111-0000-000000000000",
    ]
...
async def test_connect_fail_lua_error(executor, mocker):
    ...
    api_request_answer = [
        b"\x03",
        b"\x01",
        b"\x01\x0a\x00\x00\x00",
        b"1,4096,00000000-0000-1111-0000-000000000000",
    ]
    bootstrap_1 = [b"\x03", b"\x02", b"\x00\x03\x00\x00\x00", b"nil"]
    reader.read.side_effect = handshake_answer + api_request_answer + bootstrap_1

    mocker.patch("asyncio.open_connection", new_callable=AsyncMock, return_value=(reader, writer))

    ret = await executor.connect()
    assert ret == "Unable to connect to localhost:42069 - (MSRLuaException) "
    assert executor._socket is None
    assert isinstance(executor._socket_error, MSRLuaException)
...
async def test_malformed(executor):
    ...
    with pytest.raises(MSRLuaException):
...
async def test_code_greater_than_buffer(executor):
    ...
    with pytest.raises(ValueError, match="Single code block has length 547 but maximum is 5"):
