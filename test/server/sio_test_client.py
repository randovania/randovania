...
class SocketIOTestClient:
    ...
    Adapted from the Flask-SocketIO SocketIOTestClient.
    ...
    def __init__(self, socketio: AsyncServer, namespace=None, query_string=None, headers=None, auth=None):
        ...
        async def _mock_send_eio_packet(eio_sid, eio_pkt):
        ...
        self.queue = []
        self.acks = None
        socketio._send_packet = _mock_send_packet
        socketio._send_eio_packet = _mock_send_eio_packet
        socketio.environ[self.eio_sid] = {}
    ...
    @property
    def sid(self) -> str:
        return self.socketio.manager.sid_from_eio_sid(self.eio_sid, "/")
    ...
    async def connect(self, namespace=None, query_string=None, headers=None, auth=None):
        """Connect the client.
        ...
        if query_string:
            ...
            url += query_string
        environ = {"PATH_INFO": url, "REMOTE_ADDR": "testing"}
        environ.update(headers)

        await self.socketio._handle_eio_connect(self.eio_sid, environ)
        pkt = packet.Packet(packet.CONNECT, auth, namespace=namespace)
        await self.socketio._handle_eio_message(self.eio_sid, pkt.encode())
        sid = self.socketio.manager.sid_from_eio_sid(self.eio_sid, namespace)
        if sid:
    ...
    async def emit(self, event, *args, **kwargs):
        ...
        if isinstance(encoded_pkt, list):
            for epkt in encoded_pkt:
                await self.socketio._handle_eio_message(self.eio_sid, epkt)
        else:
            await self.socketio._handle_eio_message(self.eio_sid, encoded_pkt)
        if self.acks is not None:
            ack = self.acks
            self.acks = None
            return ack["args"][0] if len(ack["args"]) == 1 else ack["args"]

    async def send(self, data, json=False, callback=False, namespace=None):
        ...
        if json:
            ...
            msg = "message"
        return await self.emit(msg, data, callback=callback, namespace=namespace)
