...
def session_update(clean_database, mocker):
    ...
    w2 = database.World.create(session=session, name="World2",
                               uuid=uuid.UUID('d0f7ed70-66b0-413c-bc13-f9f7fb018726'), preset="{}")

    database.WorldAction.create(provider=w1, location=0,
                                session=session, receiver=w2, time=time)
