...
def test_emit_game_session_pickups_update_not_in_game(flask_app, clean_database, mocker):
    # Setup
    ...
    # Run
    world_api.emit_game_session_pickups_update(sio, world)

    # Assert
    mock_emit.assert_not_called()


def test_emit_game_session_pickups_update_one_action(
    ...
    # Run
    world_api.emit_game_session_pickups_update(sio, w1)
...
def test_game_session_collect_pickup_other(flask_app, two_player_session, echoes_resource_database,
    ...
    if exists == locations_to_collect:
        ...
        assert result == {w2}
        mock_emit_session_update.assert_called_once_with(database.MultiplayerSession.get(id=1))
