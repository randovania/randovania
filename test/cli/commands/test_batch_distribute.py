...
def test_batch_distribute_helper(mock_perf_counter: MagicMock,
    ...
    # Assert
    mock_generate_list.assert_called_once_with(expected_permalink, None, validate)
    assert delta_time == 4000
    output_dir.joinpath.assert_called_once_with("{}.json".format(seed_number))
    mock_generate_list.return_value.save_to_file.assert_called_once_with(output_dir.joinpath.return_value)
