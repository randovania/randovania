...
In order to run the tests:
   1. Run the "Getting started" step with `--full` instead of `--thin`.
   2. Find the `uv` executable. Check start_client.bat/sh for details.
   3. Run `uv run pytest`.

In order to run the server:
   1. Run the "Getting started" step.
   2. Find the `uv` executable. Check start_client.bat/sh for details.
   3. Run `uv run tools/prepare_dev_server_config.py` once.
   4. If you wish to use any Discord functionality, you'll need to create an app in Discord
   and fill both ids in `tools/dev-server-configuration.json`.
   5. Run the server and client. You can this on
