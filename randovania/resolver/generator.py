...
def generate_list(difficulty_level: int,
    ...
    print(distribute_one_item(logic, state, patches, available_pickups))
...
def find_potential_item_slots(logic: Logic,
    ...
    print(":: Potential from {} with {} actions and {} pickups at depth {}".format(
        n(state.node), len(actions), len(available_pickups), current_depth))
...
def distribute_one_item(logic: Logic, state: State,
    ...
    available_pickups_spots = frozenset(itertools.chain.from_iterable(
        item_option.available_pickups
        for item_option in potential_item_slots
    ))

    # shuffle available_pickups_spots
