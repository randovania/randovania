...
def rename_state_to_visibility(migrator: playhouse.migrate.SqliteMigrator):
    with database.db.atomic():
        database.db.execute(migrator._alter_table(migrator.make_context(), "multiplayer_session")
        ...
        database.db.execute_sql(
            "UPDATE multiplayer_session SET visibility='hidden' WHERE visibility='finished'"
        )
