...
class MajorItemState:
    ...
    def check_consistency(self, item: StandardPickupDefinition):
        ...
        if self.include_copy_in_original_location and item.original_location is None:
        ...
        if len(self.included_ammo) != len(item.ammo):
            raise ValueError(f"Mismatched included_ammo array size. ({item.name})")

        for ammo_index, ammo in zip(item.ammo, self.included_ammo):
    ...
    def bit_pack_encode(self, item: StandardPickupDefinition, reference: MajorItemState) -> Iterator[tuple[int, int]]:
        db = default_database.resource_database_for(item.game)
        if item.progression:
            main_index = item.progression[0]
        else:
            main_index = item.ammo[0]
        main_item = db.get_item(main_index)

        # original location
        if item.original_location is not None:
        ...
        # ammo index
        assert len(self.included_ammo) == len(item.ammo)
    ...
    @classmethod
    def bit_pack_unpack(cls, decoder: BitPackDecoder, item: StandardPickupDefinition, reference: MajorItemState) -> MajorItemState:
        db = default_database.resource_database_for(item.game)
        if item.progression:
            main_index = item.progression[0]
        else:
        ...
        if item.ammo:
            ...
            if custom_ammo:
                all_equal = len(item.ammo) <= 1 or bitpacking.decode_bool(decoder)
                if all_equal:
                    ammo = decoder.decode_single(db.get_item(item.ammo[0]).max_capacity + 1)
                    included_ammo = [ammo] * len(item.ammo)
                else:
                    included_ammo = [decoder.decode_single(db.get_item(item).max_capacity + 1)
                                     for item in item.ammo]
            else:
