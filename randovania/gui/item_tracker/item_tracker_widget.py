...
class TrackerElementView(abc.ABC):
    """
    Owns both the widgets for a single tracker element and how they react to a new Inventory.

    Each element kind (image/label/progress bar) gets its own subclass.
    """
...
class LabelElementView(TrackerElementView):
    def __init__(
        ...
        label_theme = theme.label_for(element)
