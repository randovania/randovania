...
class GraphRequirementList:
    ...
    def and_with(self, merge: GraphRequirementList) -> bool:
        ...
        idx: cython.int
        for other_entry in merge._other_resources:
            idx = self._find_other_idx(other_entry.first)
            if idx == -1:
                self._other_resources.push_back(pair[cython.size_t, cython.int](other_entry))
            else:
                self._other_resources[idx].second = max(self._other_resources[idx].second, other_entry.second)

        for other_entry in merge._damage_resources:
            idx = self._find_damage_idx(other_entry.first)
            if idx == -1:
                self._damage_resources.push_back(pair[cython.size_t, cython.int](other_entry))
            else:
                self._damage_resources[idx].second += other_entry.second
    ...
    def copy_then_and_with(self, right: GraphRequirementList) -> GraphRequirementList | None:
        ...
        for other_entry in right._damage_resources:
            ...
            if idx == -1:
                ...
                result._damage_resources[idx].second += other_entry.second
