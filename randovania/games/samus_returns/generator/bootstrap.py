...
def is_dna_node(node: PickupNode, config: MSRConfiguration) -> bool:
    ...
    if pickup_type == "metroid":
        if (
            (artifact_config.prefer_metroids and artifact_config.prefer_stronger_metroids)
            or (artifact_config.prefer_metroids and pickup_index not in _stronger_metroid_indices)
            or (artifact_config.prefer_stronger_metroids and pickup_index in _stronger_metroid_indices)
        ):
            return True
