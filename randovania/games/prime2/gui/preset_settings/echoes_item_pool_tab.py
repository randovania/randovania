...
class EchoesPresetItemPool(MetroidPresetItemPool):
    ...
    def _create_split_ammo_widgets(self, item_database: PickupDatabase):
        ...
        if self.game == RandovaniaGame.METROID_PRIME_ECHOES:
            beam_ammo = SplitAmmoWidget(
                parent, self._editor,
                unified_ammo=item_database.ammo_pickups["Beam Ammo Expansion"],
                split_ammo=[
                    item_database.ammo_pickups["Dark Ammo Expansion"],
                    item_database.ammo_pickups["Light Ammo Expansion"],
                ],
            )
