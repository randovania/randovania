...
def get_teleporter_connections_echoes(
    ...
    if not teleporters.is_vanilla:
        ...
        if teleporters.mode in {TeleporterShuffleMode.ECHOES_SHUFFLED}:
            ...
            connections = get_teleporter_connections(
                teleporters, game, rng, [t for t in game.get_dock_types() if t.extra.get("is_teleporter")]
            )
...
def elevator_echoes_shuffled(game: GameDatabaseView, rng: Random) -> TeleporterConnection:
    ...
    areas = {}
    for _, area, n in game.node_iterator():
        areas[n.identifier.area_identifier] = area

    def area_to_node(identifier: AreaIdentifier):
        for node in areas[identifier].actual_nodes:
